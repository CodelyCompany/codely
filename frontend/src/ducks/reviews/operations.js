...
export const Upvote = (body, token) =>
    createAction({
        endpoint: `${
        process.env.REACT_APP_BACKEND || 'http://localhost:5000'
        }/reviews/upvote`,
        ...
        types: [
        types.UPVOTE_REVIEW_REQUEST,
        {
            type: types.UPVOTE_REVIEW_SUCCESS,
            payload: async (action, state, res) => {
            const json = await res.json();
            return json;
            },
        },
        types.UPVOTE_REVIEW_FAILURE,
        ],
    });

export const Downvote = (body, token) =>
    createAction({
        endpoint: `${
        process.env.REACT_APP_BACKEND || 'http://localhost:5000'
        }/reviews/downvote`,
        ...
        types: [
        types.DOWNVOTE_REVIEW_REQUEST,
        {
            type: types.DOWNVOTE_REVIEW_SUCCESS,
            payload: async (action, state, res) => {
            const json = await res.json();
            return json;
            },
        },
        types.DOWNVOTE_REVIEW_FAILURE,
        ],
    });
